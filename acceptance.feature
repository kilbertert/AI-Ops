...
Feature: FAQ 关键词短路的歧义门

  短问题的关键词命中不足以单独作答：问题越短，单个通用词在签名中的占比越高，
  于是"今天天气怎么样"与"夏季高温酷暑暴晒天气充电…"共享一个「天气」即可满分命中。
  判据只看得见 token 重叠，分离不出这类假阳性 —— 实测「充电桩」（真问题）、
  「拔不出枪」（真问题）、「天气」（假阳性）三者重叠与包含度完全相同。因此该由
  路由意图来判定，而路由意图是每条请求本来就要算的那一个，不额外增加调用。

  Rule: 仅部分重叠标题的提问先经路由意图确认

    Scenario: 与业务无关的短问题不被当作业务 FAQ 回答
      Given FAQ 目录含「夏季高温酷暑暴晒天气充电，需要注意什么？」
      And 该问题被判为 casual 意图
      When 用户通过统一助手入口提交"今天天气怎么样"
      Then 不返回 type=faq
      And 不向用户给出该 FAQ 条目的答案

    Scenario: 案例类短问题不进入 FAQ
      Given 宣传入口的关键词未覆盖"重卡充电案例"这一说法
      And 该问题被判为 case_exploration 意图
      When 用户通过统一助手入口提交该问题
      Then 不返回 type=faq

    Scenario: 真业务问题仍然命中 FAQ
      Given 用户提交一条完整的中文业务问题
      When 该问题的措辞复述了某条目录标题
      Then 返回 HTTP 200 且 type 为 faq
      And 不查询路由意图（零模型调用）

    Scenario: 路由意图不可用时 FAQ 不因此消失
      Given 路由判定源未配置或不可达
      When 用户提交一条短业务问题
      Then 仍然返回 type=faq
      And 「无判定」不改变用户得到的内容
  ...
  Rule: 判定按意图而非按判定可用性
    ...
    Scenario: 目录标题的语言变体同样不被误伤
      Given 目录某条目有 en/de/fr/es/pt 标题变体
      And 某个变体的措辞被判为宣传或方案类意图
      When 用户提交该变体原文
      Then 仍返回该条目的 HTTP 200 type=faq
      And 全部 185 个标题变体均不查询路由意图

    Scenario: 复述标题的提问不因路由误判而失去目录答案
      Given 一条完整的目录标题问句
      And 路由意图被误判为 casual
      When 用户提交该问句
      Then 仍返回该目录条目的 type=faq 答案
...
Feature: 持续部署到服务主机
  ...
  Rule: 失败的部署把生产留在已知良好状态

    Scenario: 部署后自检失败时自动回滚
      Given 一次部署使服务无法启动
      When 部署后自检执行
      Then 主机从本次部署前的备份恢复源码并重启
      And /health 报出回滚前的版本
      And 服务为 active
      And 调用方被明确告知「已自动回滚」，而非仅「失败」

    Scenario: 自动回滚也失败时升级报告
      Given 自检失败且回滚后服务仍不 active
      When 部署结束
      Then 报告区分「已回滚」与「回滚也失败」，并给出人工介入指令

    Scenario: 依赖变更不自动同步
      Given 目标 commit 的依赖清单与主机上的不一致
      When 部署运行
      Then 在上传前停止并列出两侧差异
      And 说明这是有意的边界（改运行环境与传源码是两件事）
