...
## 41 部署决策归档（2026-09-12）

- 历史事实：旧分支 `chore/deploy-41` 的提交 `e4c58c4` 曾记录 124（41 环境）上的
  Gateway 本机只读验证：服务仅监听回环，`/health`、FAQ、媒体拒绝和 kb-service
  healthz 均符合当时预期；公网未切流，未取得 47 边缘入口管理授权。
- 决策：因 41 资源不足，部署或切流到该环境的决策作废。该验证只作为历史证据保留，
  不代表当前或候选生产入口；不把 124 上的服务状态、数据或配置变成部署承诺。
- `/kb/` 影响：120 管理域名的历史 `/kb/**` 路径仍经 cloud-gateway 指向旧地址。
  36 的 kb-service 只监听回环，直接改反代会绕过既有 OAuth/租户边界，因此本轮未改路由。
  后续须在公司网关边界批准 120→36 的受限代理、身份/租户映射、审计与回滚后，才可迁移。
- 验证：本仓文件链接与 `git diff --check` 已通过；没有宣称 `/kb/**` 已恢复，
  也没有执行 124 主机下线。
