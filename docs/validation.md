...
- 自动化检查：新增 `[project.optional-dependencies] dev`（与 `[dependency-groups] dev` 同清单）后，任务约定命令 `uv sync --extra dev` 直接通过；`uv run pytest -q` 全套 **254 项通过**，`uv run ruff check` 通过，`git diff --check` 通过，`uv.lock` 同步更新。
