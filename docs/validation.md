...
5. **比较是严格大于。** 评审抓到的第二个真缺陷：`Why Did Charging Stop Normally?` 与 q011 的
   英文标题 `Why Did Charging Stop Unexpectedly?` 共享 4/5 个 token，**双向都等于 0.80**，
   用含等号的门槛就被判自证 —— 问"正常停止"的用户拿到的是绝缘故障与过温保护的解答。
   极性只差一个 token，恰好落在门槛上。**实测 185 个目录变体没有一个恰好等于 0.80**，
   所以改成严格大于不会牺牲任何合法命中。
...
- **正解是给 cue 表补一条，不是重新放宽 FAQ 抑制集合** —— 后者已被证明会连带打掉目录自身的标题
  （"Guide" / "SOP"）。已单独建票跟踪，避免它像 #408 的前身那样只被记成"独立遗留"。
