...
### 三、选定方案：短问题的命中交给路由意图确认

`_faq_match` 返回 `(question_id, 是否自证)`：**长度达标即自证，直接作答（零模型调用）**；
**短问题才去问路由意图**。抑制集合为 `casual` / `case_exploration` / `solution_discovery`。

关键取舍（都是量出来的，不是偏好）：

1. **用长度而非分数**：分数是尺度相关的，压缩后的 en/de/fr/es/pt 标题天然短 —— 实测一个
   「重叠 ≥ 8」的绝对阈值会把 **185 个标题变体中的 91 个**送去问模型，即**每个多语言 FAQ 问题**。
...
全量 `1321 passed`；ruff check/format 通过。
...
1. **多语言标题必须问 Jev**：185 个标题变体中 136 个要问（140 个本地化变体里 135 个；
   zh 45 个里 44 个自证）。一次六语 FAQ 会话可能消耗数十次调用 ——
   **本修复把 FAQ 的多语言路径放到了按日配额上。**
2. **生产 41 的判定源当前不可用**（当日额度已耗尽，且客户流量在 12:03Z 就用完了）。
   客户端把它计成 `ROUTING_UNAVAILABLE` 并**按设计忽略**（先查 `routing` 指标桶 **0 行**才
   发现这一点 —— 见下）。

配额改变的是**本票的验收方式**，不改变代码：边际带的判定是**单调**的
（误判 →qa、正确 →faq），配额耗尽只是**回退到旧行为**，不是新故障。
