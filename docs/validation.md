...
1. **判据不是长度，而是「这问题是不是在复述某条标题的原文」。** 第一版用的是长度，
   被**全量实测否掉**：压缩后的 en/de/fr/es/pt 标题本身很短，长度分不出"短标题"与"短问句"。
   于是 `Connector Stuck? Emergency Cable Release Guide`（q010 自己的标题）也被送去问判定，
   而 Jev 从 "Guide" 一词读出 `solution_discovery`，**把目录自己的条目打掉了**
   （185 个变体中 5 个，其中 4 个属 q010）。
   改用**单变体双向包含度**：问题的词要覆盖该变体的大部分、该变体也要覆盖问题的大部分，
   且只对**最佳那一个变体**判定（不是六语并集 —— 变体匹配的是它自己的文本，不是六种译文的平均）。
   实测分离度极干净：**185 个目录变体全部 1.00**，而最高的非标题改写（`插枪扫码步骤`）只有
   **0.50**，`重卡充电案例`/`充电桩怎么拔枪？` **0.27**，`今天天气怎么样`/`天气` **0.12**。
   0.8 落在空带里。
2. **抑制集合不含 `knowledge`**：86 条真实语料里**没有任何 `knowledge` 问题落在边际带**，
   所以放宽它零收益；而它的代价可证（`充电桩怎么拔枪？` 这条真问题会被打掉）。**不因未观察到的
   风险扩大抑制面。**
...
### 六、为何不把新判据暴露成配置项（票内最后一条验收项）

票内要求「阈值/判据变更以**配置项**暴露或写明为何不需要」。**写明：不需要**，理由三条：

1. **这里的判据不是需要按业务调的量。** #383 那批阈值（`risk_at_least` 等）之所以要配置，
   是因为它们编码**业务语义**（"多确定才算涉钱"），业务方会想改。本条 `_FAQ_MARGINAL_QUESTION_SIGS`
   编码的是"问题够不够长"，跨语料观察到的带很宽（假阳性 2–6，真问题 10–12），
   取值不落在任何观测点附近 —— 把它做成配置项不会有人去调，只会多一个可漂移的面。
2. **真正可调的那部分是语义策略，不是数值。** 想改"哪类意图不该进 FAQ"，改的是
   `_FAQ_SUPPRESSING_INTENTS` 这个集合，那是**代码里的策略**，做成环境变量只会让
   "哪种意图算 FAQ"散落在部署环境里，与 #383 把策略收进代码的方向相反。
3. **代价可逆且已界定。** 判据只影响"要不要多问一次判定"，不影响答案内容；
   判定不可用时行为不变（见第三条取舍）。因此没有"必须能在生产上紧急改回来"的压力 ——
   那才是配置项的主要用途（对照 #402 的 `AIOPS_GATEWAY_ROUTING_RISK_ALWAYS_ASKS`，
   那条有实测的回滚路径，因为改的是**用户可见**的追问行为）。

若日后业务方确实要调，正确的落点是 `RoutingThresholds` 旁边的同类配置，并按 #402 的做法
补一条"可从环境读到"的测试（`test_every_routing_setting_is_reachable_from_the_environment`
就是为防那种漂移而加的）。
...
| 检查 | 结果 |
|---|---|
| A. 185 个目录标题变体（含 en/de/fr/es/pt） | **185/185 仍走 FAQ；其中 185 免模型、问了判定 0 次、失败 0** |
| B. 缺陷集 | `今天天气怎么样`/`今天天气如何`/`天气` → casual → qa；`重卡充电案例` → case_exploration → qa；其余不命中 |
| C. 真业务问题 | 全部仍 FAQ（knowledge / report_fault 不抑制） |
| D. ASR 碎片与寒暄 | 均不命中 FAQ |
| E. 86 条真实语料 | 旧 `{qa:85, faq:1}` → 新 `{qa:86}`，**唯一变化 = `重卡充电案例` faq→qa**（假阳性），**零真业务问题受损** |
| Jev 调用失败 | **0** |

**这次实测发现并修掉了合并版本里的一个真回归**（见第三条取舍）：q010 的四个语言变体
...
**状态：未取得。** 额度耗尽 → 无法在 41 上跑完「185 个标题变体 + 86 条语料」的实测。
