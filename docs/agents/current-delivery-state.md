...
- **便携客户端与工作区级设备注册**（设备登记、设备令牌、便携包分发）。
  **产品侧已决定退役**，且**已观察**到该入口无使用：三台现役网关上
  **已观察的网关均无设备注册**（其中一台 `runs` 也为 0，见 `../validation.md`
  的逐台记录；**未取得证据的网关不计入**）。
  **退役本身尚未立票**。**在此之前**：该路径的订单可见性**不受 #423 影响**，
  ...
  > **已被标记 DEPRECATED**（PRD #23，2026-08-31）；**受损范围直连（`ScopedSources`）
  ...
  **注意（结论强度）**：观察范围是**三台现役网关**，不等于"公司内无任何便携包在跑"。
