...
>
> **最后核验**：2026-09-23 18:05（Asia/Shanghai）
> **仓库基线**：读取时在 canonical checkout 执行 `git rev-parse origin/main`。
> 最近固化的状态索引变更为 PR #404（CD 部署完成与对 CD 构建的重新验收；
> 此前依次为 #403/#401/#400/#399）。
...
| 交付项 | 当前状态 | 证据/边界 |
|---|---|---|
| 意图路由 | 判定源已切到 Jev，41 已配置并验收 | 寒暄/FAQ/订单诊断/缺订单号澄清/宣传路由见 `validation.md` 的 INTENT-01..08；`smart_diagnosis` 缺订单服务端保护已由 PR #253 修复并公网复测。`今天天气怎么样` 的 FAQ 关键词假阳性仍是独立遗留。**2026-09-23（PRD #383）：判定三字段改由 Jev 类型化判定产出**（`docs/validation.md` 的 #392 节）；阈值来自 #390 标定且可配置；Jev 未配置时仍走原模型分类器。**41 已配置并验收**；寒暄走 202 零阶作业。**真实流量复标（86 条）更正了一处结论**：那条高风险追问规则在真实流量上**仍未触发**（`would_ask=0/86` —— Jev 认出涉钱，但同时对判断极度自信）；#400 记录的「首次触发」针对的是**手写示例问句**。**规则语义已改为「高风险一律追问」**（方案 A，2026-09-23）：`risk>=0.5` 即追问，去掉 confidence 条件。**实测增量是 3 条而非 5 条** —— 既有还有一条**关键词**守卫`_HIGH_RISK_ORDER_CUES` 在分类器之前就拦下其中 2 条（它要订单号、方案 A 要上下文）。配置项 `AIOPS_GATEWAY_ROUTING_RISK_ALWAYS_ASKS`（默认 true）**已在 41 实测可一键还原**（设 false → 同一问句不再追问 → 恢复默认）。**方案 A 已由 CD 部署 41 并公网验收**（41 运行 `0.1.0+4a1ac3a4c041`，`/health` 可对照修订）；未决：关键词守卫（要订单号）与方案 A（要上下文）是否合并。详见 `validation.md` 的「Jev 阈值按真实流量复标」。 |
...
| 41 的部署方式 | **已改为受门控的自动化 CD**（2026-09-23） | 合并 main 且改动含 `src/**` 时，`cd.yml` 自动准备部署，经 `production-41` 环境的**人工审批**后执行 `deploy-41.sh`。首次验收四条判据全通过（触发 / 门控拦住 / 批准后部署成功且 `/health` 带本次 sha / Deployments 记录）。部署后 `/health` 报 `0.1.0+<commit-short-sha>`，可自证跑的是哪个 commit；失败时主机侧自检并**自动回滚**。**依赖变更不在 CD 范围**（清单不一致时拒绝部署），走 `env-41-dependency-update.md` 的人工流程。证据见 `validation.md`。 |
