...
迁移命令已在任务分支实现：`aiops admin migrate-shortcuts --db <gateway.db>`，支持
`--dry-run`、幂等平台默认创建和失败草稿清理；原租户发布版本不删除。当前 41 尚未部署该
提交，真实迁移前必须精确备份 SQLite，按 `qa-plan.md` 的 SHORTCUT-MIGRATION-01..05
执行并保留日志。任何本地 fixture 或 SQLite 检查都不能替代双租户公网业务验收。
