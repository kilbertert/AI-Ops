...
- **便携客户端 / 设备直连路径**（`/diag/*` + 工作区级设备注册）。**产品侧已决定退役**，
  且**已观察**到该入口无使用：三台现役网关上 `devices` 与 `runs` **均为 0**。
  **退役本身尚未立票** —— 它包含删文档、改 `docs/gateway.md` 的产品语义
  （现写"设备不绑租户即可查任意订单"）、清理 `/diag/*` 契约，是**一件事、一个 owner**。
  **在此之前**：该路径的订单可见性**不受 #423 影响**，也不应被当作"已安全"。
  **注意**：观察范围是**三台现役网关**，不等于"公司内无任何便携包在跑"。
