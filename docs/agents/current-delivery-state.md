...
> 最近固化的状态索引变更为 PR #409（交付状态核查：CD 部署史与 FAQ 假阳性；
> 此前依次为 #404/#403/#401/#400/#399）。

## 一分钟结论

### 当前无正在实施的任务（有 4 张待办票，均未开工）

**2026-09-23**：上一版这里写着「当前进行中的 #247」，而 **#247 已于 2026-09-16 关闭** ——
标题与实际不符，会让读者以为有活在跑。现改为如实陈述。

「进行中」指**已有人在做**；下面四张是**已建票但尚未开工**，因此不在进行中。
...
- #408 FAQ 关键词判据把无关短问题误判为业务 FAQ（**已实测复现**，见下）。

**已交付且已部署的最近一批**（PRD #383 及其后续）：

迁移命令（#247，PR #251）早前已合并并部署；快捷动作迁移、判定切到 Jev、方案 A 高风险追问
均已由 CD 部署到 41（`0.1.0+4a1ac3a4c041`）。
