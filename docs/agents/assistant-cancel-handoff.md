...
> **行为变更（2026-09-23，#391）：寒暄类提问现在会进等待态。**
> 此前寒暄（"你好"、"谢谢"）由分类器**同步直答** `200`，因此**不锁**输入框。现在它与其它
> 一般提问走同一个零阶 QA 作业，返回 **`202` + `type=qa`** —— **会锁、要轮询**。
> 上表的判别规则本身没变（仍是 `type` + HTTP），变的是寒暄落到了哪一行：
> 它从 `200` 那一侧移到了 `202` 那一侧。**不要为寒暄写一套"同步直答"的特殊渲染分支**，
> 它不会再走到那里。
>
> **不只是"晚一点到"——字段也变了。** 旧响应内联 `result.text`，**没有** `qa_id`；
> 新响应**新增 `qa_id` 与 `retry_after_ms`**，并**不再带内联 `result.text`**（答案在轮询到
> 终态后出现在 `result` 里）。按"形状不变、只是晚一点"去实现，会在读 `result.text` 时拿到
> `null`。**按上表 `qa` 行处理即可**：拿 `qa_id`、按 `retry_after_ms` 轮询、终态渲染 `result`。
