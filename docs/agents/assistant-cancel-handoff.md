...
| 入口响应 | HTTP | 等待态 | 前端动作 |
|---|---:|---|---|
| `type=qa`（自由提问 / 宣传卡片） | 202 | **是** | 整个输入区替换为等待态；按 `retry_after_ms` 轮询 `GET /v1/assistant/questions/{qa_id}` |
| `type=diagnosis`（订单诊断） | 202 | **是** | 同上；轮询 `GET /v1/standard/diagnoses/{diagnosis_id}` |
...
`qa_id` 不存在、**不属于当前调用者**（跨租户 / 跨作用域）、已过保留期被清扫 —— 三种情况返回
**同一个** `404 QA_NOT_FOUND`。这是刻意的：取消不能成为探测「哪些 `qa_id` 真实存在」的通道。
**不要**拿它区分「作业不存在」和「取消失败」。
