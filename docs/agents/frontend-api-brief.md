...
| code | 含义 | 前端处理 |
|---|---|---|
| `DIAGNOSIS_ORDER_OUT_OF_SCOPE` | 订单不在当前调用者租户内（越权）。**纵深防御信号，正常配置下不会出现**（见下） | 提示"该订单不属于当前账号"，引导换本账号订单；重试不会改变结论 |
...
> **行为变更（2026-09-23，#408）：接近但不等于目录标题的提问，其 FAQ 命中不再一律同步直答。**
> 关键词判据只看得见 token 重叠，而短问句的签名短，一两个通用词就能对某条目录标题
> 达到满分包含度 —— 用户问「今天天气怎么样」会收到一条**充电高温注意事项**。
> 修复后：**只是接近、而非等同于**某条目录标题原文的提问，会先经路由意图确认，
> 被判为 `casual` 时**不再走 FAQ**，改走 `qa`（202，需轮询）。
> 判据是"问题与某条标题的语言变体**token 集合相等**"，与问题长短无关 ——
> 请前端**不要**按长度做任何假设（`Why Is Charging Power Faster Than Advertised?`
> 有 7 个词，仍会被拦下）。
>
> 对前端的影响只有一处：**这类提问的响应从 `200 type=faq` 变成 `202 type=qa`。**
>
> - **点击推荐问题不受影响** —— 它走 `POST /v1/faq/answer`（带 `question_id`），
>   不经过关键词判据，仍是同步 `200`。
> - **完整业务问句不受影响** —— 与某条目录标题原文 **token 集合相等**即直接作答，仍是零模型调用的
>   同步 `200 type=faq`；六种语言的标题变体同样**全部**走同步路径（实测 185/185，
>   且全部无需模型调用）。
> - **FAQ 页与推荐列表不受影响** —— 它们读 `recommendations`/`catalog`，与本次判据无关。
> - 请前端按既有 `type` 分派逻辑处理（`faq` → 直接渲染；`qa` → 按 `retry_after_ms` 轮询），
>   **不要假设「问的是 FAQ 里的内容就一定拿 200」**。

> **行为变更（2026-09-23，#391）：寒暄类提问现在返回 `202` + `type=qa`。**
> 此前"你好"、"谢谢"这类寒暄由分类器**同步直答** `200`（不在上表任何一行，属于 `qa` 的
> 一个同步特例）。现在它与其它一般提问走同一个零阶 QA 作业：**`202` + `qa_id`，要轮询，
> 并锁输入框**。前端按上表的 `qa` 行处理即可，**不要为寒暄保留"同步直答"分支**。
