...
- 模型回答的语言注入已在代码层支持（qa 与订单诊断的模型提示词注入输出语言指令——`text`/summary/root_cause 等人类可读内容用 `Accept-Language` 对应语言书写，知识库片段与证据原文保持原样）。线上实测证据目前覆盖：FAQ 确定性面（L1/L2/L3）与 qa 路由语言透传（`202 type=qa language=en`）；qa 完成态与诊断链路的多语言完成态实测待供应商恢复后进行（见 #218），完成后在此更新实测日期。
