...
> **行为变更（2026-09-23，#408）：短问句的 FAQ 命中不再一律同步直答。**
> 关键词判据只看得见 token 重叠，而短问句的签名短，一两个通用词就能对某条目录标题
> 达到满分包含度 —— 用户问「今天天气怎么样」会收到一条**充电高温注意事项**。
> 修复后：**只是部分重叠**某条目录标题、而非复述其原文的提问，会先经路由意图确认，
> 被判为 `casual` / 案例 / 方案类时**不再走 FAQ**，改走 `qa`（202，需轮询）。
>
> 对前端的影响只有一处：**这类短问句的响应从 `200 type=faq` 变成 `202 type=qa`。**
>
> - **点击推荐问题不受影响** —— 它走 `POST /v1/faq/answer`（带 `question_id`），
>   不经过关键词判据，仍是同步 `200`。
> - **完整业务问句不受影响** —— 复述目录标题原文即直接作答，仍是零模型调用的同步 `200 type=faq`；
>   六种语言的标题变体同样**全部**走同步路径（实测 185/185）。
