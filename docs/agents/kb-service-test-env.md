...
> 状态：当前有效，**2026-09-18 修订**（KB 栈集中在移动云 36；41 经受限隧道复用，
> **管理后台 `/kb/**` 亦已于本日归并到同一套 36**，见 §0；95 入口保持独立，见 §1）。
...
【管理后台链路（2026-09-18 起已并入 36，与 AI-Ops 同一套）】
admin.qumall.qushiyun.com（120 Nginx）/kb/**
    → 120 本机隧道 127.0.0.1:9381（systemd kb-36-tunnel.service）
    → 36:127.0.0.1:9380（受限 authorized_keys + permitopen 白名单）
    → 与 AI-Ops 网关使用的是**同一个 kb-service / 同一套 RAGFlow**
```

### 2026-09-18 打通：后台知识库页与智能体平台已统一到 36

**背景（重要）**：曾长期存在**两套互相独立的 RAGFlow 栈**，二者是 2026-09-09 停机后
从 124 复制到 36 的两份分叉副本（数据集名完全重合）。打通前的实际状态：

| 入口 | 指向 | 内容 |
|---|---|---|
| 后台知识库页 `/kb/` | 124.243.178.156:9380 | 11 个数据集，**全是测试残留**（`p3-smoke`/`table-demo`/`ui-probe-status` 等） |
| AI-Ops 网关 | 41:29380 隧道 → **36** | 15+ 数据集；**3 个已发布智能体绑定在这里** |

后果：**在后台页建的知识库，智能体平台看不到**（反之亦然）——这就是"未打通"的根因。

**统一方向：全部归并到 36**（不搬数据）。理由：内容是活的、已绑在跑的智能体；
36 的租户都配了 VISION 模型 `qwen3-vl-plus@maas@Tongyi-Qianwen`（视频/图片解析的前提，
124 缺此项）；后台只需改后端地址，不动任何已有数据与绑定。

**已完成的变更（120 生产 Nginx）**：
- 新增 36 侧受限账号 `nginx-kb-tunnel`（`nologin` +
  `restrict,port-forwarding,permitopen="127.0.0.1:9380",command="/bin/false"`），
  独立密钥（不复用 41 的）。
- 120 新增 `kb-36-tunnel.service`（加固参数同 `aiops-36-kb-tunnel`），
  本地监听 `127.0.0.1:9381` → 36:9380。
- 改 `/www/server/panel/vhost/rewrite/admin.qumall.qushiyun.com.conf`：
  `location ^~ /kb/` 由 `124.243.178.156:9380` 改为 `127.0.0.1:9381`；
  `location = /kb/healthz` 由 `127.0.0.1:9380`（**120 本机该端口是 dead 的
  kb-service.service，等于假探活**）改为 `127.0.0.1:9381/healthz`。
  备份在 `/root/nginx-backup/admin.qumall.qushiyun.com.conf.<ts>.bak`。
- 回滚：恢复上述备份 → `nginx -t` → `systemctl reload nginx`。

**租户映射（关键约束）**：RAGFlow 按租户隔离，后台登录身份决定能看到哪些库。
- `1783022023241633792` = **「小趋充电」**，是**真实 UPMS 租户**，后台顶部租户切换器
  可选；其 `canary-media-0911`（2 文档/2 分段）**同时**绑在已发布的 `canary-客服` v5 上
  —— 这是验证打通的最佳样本。
- `1942105476598861824` 与 `1899282205965029376` **不是 UPMS 租户**（318 个租户里查无），
  属 KB canary 自造 id，**后台租户切换器里永远选不到**。材料知识库
  `8701c742b28111f18637d95f7710e3a3`（「41-客服知识库」，12 文档含 3 个原文件 + 9 张
  宣传图）属于前者，故**只能在 API/智能体侧使用，不能在后台页管理**。

**仍待迁移（未在本次授权范围）**：`canary-宣传案例` v2 与另一个 `canary-客服` v3 绑定的
KB 属非 UPMS 租户；若要它们也能在后台管理，需给 UPMS 真实租户重新建库并重绑。
...
通过 41 的受限回环隧道复用。管理后台 `/kb/**` 已于 2026-09-18 归并到同一套 36
（经 120 本机隧道 9381，见 §0），不再指向 124；仍经公司网关做 OAuth/租户鉴权，
只是后端目标从已分叉的 124 栈换成权威的 36 栈。
...
`宣传.docx`（5512697B）、`新加坡无人电动巴士.mp4`（82348365B）、`重卡充电.mp4`
（15520567B）。

> **2026-09-18 更正**：本文早前把 `重卡充电.mp4` 记作 370MB —— 与实物不符，实际
> 15.5MB（已按字节核对）。此前"370MB 视频是 Range 的好测试对象"的说法随之作废；
> 现存最大视频是 82MB 的 `新加坡无人电动巴士.mp4`。

这三个文件**已在 36 入库**（见 §0「2026-09-18 打通」）：属于租户
`1942105476598861824` 的知识库 `8701c742b28111f18637d95f7710e3a3`（「41-客服知识库」），
12 个文档全部 `run=DONE`，除上述 3 个文件外还含 9 张宣传图（本地目录没有，只在 36
的数据集里）。
...
2. ~~kb-service 补图片透传端点~~ **已完成**（2026-09-11 部署 36 并实测；源码记录在
   Playground 沙箱仓库，见 §1）。
3. ~~公司侧待办：admin `/kb/` 历史目标 124 需复核后再动~~ **已于 2026-09-18 完成**
   （见 §0）：120→36 的受限代理、独立密钥、身份与租户映射、备份与回滚路径均已落地并
   端到端验证。原"不得以直连 36 回环端口方式替换"的约束仍然成立——本次正是走**受限
   隧道**而非直连回环，OAuth/租户鉴权经公司网关保留不变。

   剩余公司侧事项（非本仓库、未完成）：
   - UPMS 仍需建 `ROLE_AGENT_ADMIN` 角色族以启用管理面 HTTP 化；
   - 业务方轮换被封锁的 alibaba-maas key；
   - 41 Gateway 当前默认使用 `canary-dashscope`，36 RAGFlow embedding 仍需恢复可用的
     百炼账户或替换 provider（2026-09-18 复验解析已成功，欠费似已缓解，但仍需正式确认）。
