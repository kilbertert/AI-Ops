...
| INTENT-02 | 本地 dev | 无天气工具 | 提交“今天天气怎么样” | 不检索业务 KB；明确无法查询实时天气；不伪造天气 | **原记录有误，已于 2026-09-23 更正**。此处曾写 PASS 并注明「变体『你是谁呀』」，即用另一句回答通过了本行；而本行指定的操作（“今天天气怎么样”）当时被 FAQ q026 关键词误命中，`acceptance.feature` 的 `无实时工具时天气问题诚实说明能力边界` 自写入起一直未通过。**该记录把变体的结果写成了原题的结果。** 修复见 #408 与 INTENT-20 |
...
| INTENT-09 | 本地 dev | 已授权订单 2096164064667852801 | 提交英文内嵌订单号“Please check charging anomalies for order (2096164064667852801)” | `202 type=diagnosis`，order_no_extracted 为该订单号 | PASS：202 diagnosis，order_no_extracted 正确（修复前为 `qa`——英文取自句首单词 `Please`） |
...
| INTENT-21 | 41 实机（真实 Jev） | #408 修复后 | 提交“重卡充电案例” | 不返回 type=faq | PASS：intent=case_exploration → 抑制；**该题是 86 条真实语料中唯一的 type 变化**（旧 {qa:85,faq:1} → 新 {qa:86}） |
| INTENT-22 | 41 实机（真实 Jev） | #408 修复后 | 提交完整中文业务问句（“充电桩怎么拔枪？”等 11 条） | 仍返回 type=faq | PASS：11/11 仍 FAQ（knowledge/report_fault 均不抑制）。另有 `客服电话是多少` 未命中任何条目 → qa，属既有行为 |
...
## 回答载荷契据封口 QA（ANSWER-PAYLOAD，PRD #361 子票 T5/#367）

| ID | 环境 | 前置条件与数据 | 有序动作 | 预期可观察结果 | 清理/证据 |
|---|---|---|---|---|---|
| ANSWER-PAYLOAD-01 | 本地 dev | 四个回答面的生产源码 | 解析 `src/aiops_diagnostics/*.py`，按 import 绑定（含别名）枚举 `answer_chinese_leak` 的每个调用点，断言首个实参是契约构造结果或已被证明的字符串 | 四个调用点全部合规；出现裸 list / 裸 dict / 未证明的读值即报出文件与行号 | `tests/test_answer_caller_shapes.py::test_every_finalisation_point_hands_the_guard_the_contract_payload` |
