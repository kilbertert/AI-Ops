...
def test_device_parameter_rejects_injection_characters_before_query() -> None:
    ...
    assert source.index("isSafeValue(deviceValue)") < source.index("jdbcTemplate.queryForList(deviceSql")
...
def test_tenant_id_is_optional_and_rejected_before_query_if_unsafe() -> None:
    ...
    assert source.index("isSafeValue(queryTenantId)") < source.index("jdbcTemplate.queryForList(deviceSql")
...
def test_blank_device_keys_are_normalized_before_exactly_one_guard() -> None:
    ...
    assert source.index("boolean hasDeviceCode = normalizedDeviceCode != null;") < source.index(guard)
