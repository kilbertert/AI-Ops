...
- `cloud-charging-pile-web/src/main/java/com/qushiyun/cloud/charging/pile/web/controller/DiagQueryController.java`
  ...
  `GET /diag/redis-stream` T5 同步队列检查：仅放行 `third.order.sync.queue` / `third.order.sync.notify.queue`，
  返回有界 `XREVRANGE` 窗口、Stream 长度、消费组滞后与按 `order_no` 的匹配计数，`max_messages` 强制上限 1000。
...
- 本仓库无 JDK/Spring 工具链，未编译或部署该 Java 服务；这里只通过 `tests/test_diag_order_contract.py`
  与 `tests/test_diag_redis_stream_contract.py` 守护工件中的外部契约关键点，不等价于真实环境验收。
