...
- 管家端授权链的审查修复——C→B 映射采用前核对记录自带的 `userId`（原先只核对租户后再盖写，同租户但绑在别的 C 端用户名下的记录会被采用，把对方的运营商站点集合当成本会话可见范围）、歧义判定移到租户筛选之后（「本租户唯一主体 + 一条跨租户记录」不再被判歧义）、站点集合上限防线前移到查询里（`SITE_SCOPE_MAX_ROWS` 与 `MAX_SCOPE_IDS` 相等导致 `LIMIT 1000` 无法区分完整与截断，改为多取一行、超界抛 `scope.too_large`）、SSH 隧道只请求用到的那一条转发（多要转发会让 `ExitOnForwardFailure=yes` 杀掉 ssh，整个管家端授权链因无关拒绝而失败关闭）；另将四个测试片共用的替身收敛到 `tests/operator_support.py`，修绿 CI 的 `ruff format --check` (#431)
